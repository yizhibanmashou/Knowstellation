...
import {
  ...
  buildConceptDetailsChatRequest,
...
} from '../src/shared/services/llmClient.ts';
...
test('concept detail request asks for one focused sidebar paragraph', () => {
  const request = buildConceptDetailsChatRequest({
    chapterId: 'chapter18',
    conceptId: 'concept_formula_18_3_defined_mu_t',
    viewId: 'concept_formula_18_3_defined_mu_t',
    name: 'Mean Liability Before Selection',
    symbol: '\\mu_t',
    conceptType: 'quantity_concept',
    definition: '第 t 代选择前 liability 分布的均值。',
    formula: {
      id: 'formula_18.3',
      label: 'Formula 18.3',
      latex: '\\mu_t = ...',
      section: 'Response to selection',
    },
    prerequisiteConcepts: [{ name: 'Liability Distribution', definition: '个体潜在责任值的分布。' }],
    successorConcepts: [{ name: 'Mean Liability After Selection', definition: '选择后群体责任值均值。' }],
    language: 'zh',
  });
  const payload = JSON.parse(request.messages[1].content);

  assert.equal(payload.task, 'concept_detail');
  assert.equal(payload.concept.symbol, '\\mu_t');
  assert.equal(payload.supporting_formula.label, 'Formula 18.3');
  assert.match(payload.output_schema.explanation, /一段中文概念解读/);
  assert.match(payload.output_schema.explanation, /90-150/);
});
...
test('concept detail parser accepts a single explanation paragraph', async () => {
  const result = await __llmClientTestUtils.postChatCompletion(
    buildConceptDetailsChatRequest({
      chapterId: 'chapter18',
      conceptId: 'concept_formula_18_3_defined_mu_t',
    ...
    __llmClientTestUtils.validateConceptDetails,
    async () =>
      new Response(
        JSON.stringify({
          choices: [{ message: { content: '{"explanation":"第 t 代选择前 liability 分布的均值描述选择发生前群体在潜在责任值尺度上的中心位置。在 Formula 18.3 中，\\\\mu_t 不是孤立均值，而是后续比较选择前后分布移动的基准。"}' } }],
        }),
        { status: 200, headers: { 'Content-Type': 'application/json' } },
      ),
  );

  assert.match(result.explanation, /Formula 18.3/);

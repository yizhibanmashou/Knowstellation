...
import { compactMathText, latexToReadableCandidates, symbolRequiresOverline } from '../src/shared/utils/mathSymbolMatching.ts';
...
test('latexToReadableCandidates keeps scripted overline symbols specific', () => {
  ...
  const circCandidates = latexToReadableCandidates('\\overline{z}_{i}^{\\circ}');
  assert.ok(circCandidates.includes('z_i∘'));
  assert.ok(!circCandidates.includes('z_i'));
  assert.ok(!circCandidates.includes('zi'));
...
test('compactMathText normalizes KaTeX star scripts', () => {
  assert.equal(compactMathText('μ∗'), 'μ*');
  assert.ok(latexToReadableCandidates('\\mu^{*}').includes('μ*'));
});
